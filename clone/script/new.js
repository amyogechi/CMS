...
//   userInfoDiv.style.marginLeft = '2px'
//   userInfoDiv.textContent = `Login as: ${userEmail}`;
// }
...
//     // Collect form data using FormData (supports file upload)
...
//       formData.append("regno", document.getElementById("matric").value.trim());
...
//       Swal.fire("Error", "Title and details are required!", "error");
//       return;
//     }


//     if (!isAnonymous) {
//       if (!formData.get("name") || !formData.get("regno") || !formData.get("email") || !formData.get("department")) {
//         Swal.fire("Error", "All personal fields are required!", "error");
//         return;
//       }
//     }
...
//   const tableBody = document.getElementById("complaint-body");
//   if (!email) return;

//   tableBody.innerHTML = `<tr><td colspan="2">Loading...</td></tr>`;
//   try {
//     const res = await fetch(`/api/user-complaints/${email}`);
...
//       });
//     } else {
//       tableBody.innerHTML = `<tr><td colspan="2">No complaints found.</td></tr>`;
//     }
//   } catch (err) {
//     console.error(err);
//     tableBody.innerHTML = `<tr><td colspan="2">Error loading complaints.</td></tr>`;
//   }
// }
...
if (userEmail) {
  document.getElementById("userInfo").textContent = `Login as: ${userEmail}`;
}
  ...
  // Toggle personal info for anonymous submissions
  anonymousCheckbox.addEventListener("change", () => {
    if (anonymousCheckbox.checked) {
      personalInfoSection.classList.add("hidden");
    } else {
  ...
  complaintForm.addEventListener("submit", async (e) => {
    ...
    if (!isAnonymous) {
      ...
      // For anonymous, still include email to track user
    ...
    try {
      ...
      if (res.ok) {
        ...
        complaintForm.reset();
        personalInfoSection.classList.remove("hidden");
        fetchUserComplaints(); // refresh table
      } else {
        Swal.fire("Error", result.message, "error");
      }
    } catch (err) {
  ...
  async function fetchUserComplaints() {
    ...
    try {
      ...
      console.error(err);
      tableBody.innerHTML = `<tr><td colspan="4">Error loading complaints.</td></tr>`;
    }
  }
