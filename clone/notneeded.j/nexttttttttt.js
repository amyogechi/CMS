...
//     if (anonymousCheckbox.chec// // Configuration constants
...
function validateForm() {
  ...
  if (!confirmPassword) {
    addFieldError('confirmPassword', ERROR_MESSAGES.PASSWORD_REQUIRED);
    isValid = false;
...
async function userRegistration() {
    ...
    let formData = { role, username, email, password };
    ...
    if (role === 'student') {
        ...
        formData.regno = document.getElementById('regno').value.trim();
    }

    try {
        // Example: call API (replace with your actual API call)
  ...
  try {
    ...
    if (data.exists) {
      ...
      if (adminOption) {
        ...
        adminOption.textContent = 'Admin (Unavailable)';
      }
      // Optionally show a note
      const info = document.createElement('small');
      info.style.color = '#666';
      info.style.display = 'block';
      info.textContent = 'Admin registration disabled until the current admin is removed.';
      roleSelect.parentNode.appendChild(info);
    }
  } catch (err) {
...
// ====== JS CODE ======
    ...
    function togglePassword(id, iconSelector) {
      const field = document.getElementById(id);
      const icon = document.querySelector(iconSelector);
      if (field.type === 'password') { field.type = 'text'; if(icon) icon.textContent='🙈'; }
      else { field.type='password'; if(icon) icon.textContent='👁️'; }
    }

    function handleRoleChange() {
      if(roleSelect.value === 'admin') studentFields.classList.add('hidden');
      else studentFields.classList.remove('hidden');
    }
    ...
    function showPasswordStrength(password) {
      const strengthElement = document.getElementById('passwordStrength');
      if(!password){ strengthElement.innerHTML=''; return; }
      ...
      strengthElement.innerHTML=`
        <div class="password-strength-bar">
          <div class="strength-fill" style="width:${(strength/5)*100}%; background-color:${colors[level]}"></div>
        </div>
        <span style="color:${colors[level]}">${levels[level]}</span>
      `;
    }

    document.getElementById('password').addEventListener('input', e => showPasswordStrength(e.target.value));
    ...
    form.addEventListener('submit', async e=>{
      ...
      if(data.password !== data.confirmPassword){
        Swal.fire({icon:'error',title:'Error',text:'Passwords do not match'}); return;
      }
      ...
      try{
        ...
        const result = await response.json();
        if(response.ok){
          Swal.fire({icon:'success',title:'Success',text:result.message,timer:2000,showConfirmButton:false}).then(()=>window.location.href='/');
        } else {
...
function handleRoleChange() {
  ...
  const inputs = ['firstname', 'lastname', 'regno'].map(id => document.getElementById(id));
...
function showPasswordStrength(password) {
  ...
  if (!strengthElement) {
    strengthElement = document.createElement('div');
    strengthElement.id = 'passwordStrength';
    const container = document.getElementById('password').closest('.form-group');
  ...
  if (/[a-z]/.test(password)) strength++; else feedback.push('One lowercase letter');
  if (/[A-Z]/.test(password)) strength++; else feedback.push('One uppercase letter');
  if (/\d/.test(password)) strength++; else feedback.push('One number');
  if (/[!@#$%^&*(),.?":{}|<>]/.test(password)) strength++; else feedback.push('One special character');
  ...
  strengthElement.innerHTML = `
    ...
    <span style="color:${colors[level]}">${levels[level]}</span>
    ${feedback.length ? `<small>Missing: ${feedback.join(', ')}</small>` : ''}
  `;
}
...
function validateForm() {
  ...
  if (!role) { addFieldError('role', ERROR_MESSAGES.ROLE_REQUIRED); isValid = false; }
  ...
  if (role === 'student') {
    ...
    if (!firstname) { addFieldError('firstname', ERROR_MESSAGES.FIRSTNAME_REQUIRED); isValid = false; }
    else if (firstname.length > VALIDATION_CONFIG.MAX_NAME_LENGTH) { addFieldError('firstname', ERROR_MESSAGES.NAME_TOO_LONG); isValid = false; }

    if (!lastname) { addFieldError('lastname', ERROR_MESSAGES.LASTNAME_REQUIRED); isValid = false; }
    else if (lastname.length > VALIDATION_CONFIG.MAX_NAME_LENGTH) { addFieldError('lastname', ERROR_MESSAGES.NAME_TOO_LONG); isValid = false; }

    if (!regno) { addFieldError('regno', ERROR_MESSAGES.REGNO_REQUIRED); isValid = false; }
    else if (!VALIDATION_CONFIG.REGNO_PATTERN.test(regno)) { addFieldError('regno', ERROR_MESSAGES.REGNO_INVALID); isValid = false; }
  }

  if (!username) { addFieldError('username', ERROR_MESSAGES.USERNAME_REQUIRED); isValid = false; }
  else if (username.length > VALIDATION_CONFIG.MAX_USERNAME_LENGTH) { addFieldError('username', ERROR_MESSAGES.USERNAME_TOO_LONG); isValid = false; }

  if (!email) { addFieldError('email', ERROR_MESSAGES.EMAIL_REQUIRED); isValid = false; }
  else if (!isValidEmail(email)) { addFieldError('email', ERROR_MESSAGES.EMAIL_INVALID); isValid = false; }

  if (!password) { addFieldError('password', ERROR_MESSAGES.PASSWORD_REQUIRED); isValid = false; }
  else if (password.length < VALIDATION_CONFIG.MIN_PASSWORD_LENGTH) { addFieldError('password', ERROR_MESSAGES.PASSWORD_TOO_SHORT); isValid = false; }

  if (!confirmPassword) { addFieldError('confirmPassword', ERROR_MESSAGES.PASSWORD_REQUIRED); isValid = false; }
  else if (confirmPassword !== password) { addFieldError('confirmPassword', ERROR_MESSAGES.PASSWORDS_MISMATCH); isValid = false; }

  if (password) showPasswordStrength(password);
...
function setupFieldHandlers() {
  const fields = ['role','firstname','lastname','regno','username','email','password','confirmPassword'];
  fields.forEach(id => {
    const field = document.getElementById(id);
    if (!field) return;
    field.addEventListener('focus', () => clearFieldError(id));
    field.addEventListener('input', () => {
      clearFieldError(id);
      if (id === 'password') showPasswordStrength(field.value);
    });
    if (id === 'role') field.addEventListener('change', handleRoleChange);
  });
}

// ================= SUBMISSION WITH SWEETALERT =================
  ...
  try {
    ...
    if (response.ok) {
      Swal.fire({
        ...
        timer: 2000,
        showConfirmButton: false
      }).then(() => { resetForm(); window.location.href = '/'; });
    } else {
      if (response.status === 403) {
        Swal.fire({ icon: 'error', title: 'Admin Exists', text: result.message });
      } else if (response.status === 400) {
        Swal.fire({ icon: 'error', title: 'User Exists', text: result.message });
      } else {
        Swal.fire({ icon: 'error', title: 'Oops...', text: result.message, confirmButtonColor: '#d33' });
      }
    }
  } catch (err) {
    Swal.fire({ icon: 'error', title: 'Network Error', text: 'Something went wrong. Please try again later.' });
  }
}
